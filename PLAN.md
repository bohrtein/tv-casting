...
MP4, served over plain LAN http) had worked. Kept everything done since:
the Matrix-synced companion, `tv-receiver` logging and the static idle
background. The server still had the resolver's `.env`, `node_modules`
and systemd unit, so pulling brings it back up. The old rewatch-cache
entries left in the primary clone were merged into the App Hub clone's
cache, which is the one the unit runs from.
