...
const ContentPolicy = require('../../companion/js/content-policy');
...
function libraryFields(body) {
  ...
  if (body.metadata) {
    ...
    fields.metadata = {};
    for (const key of ['id', 'type', 'name', 'poster', 'background', 'description', 'videoId', 'episodeTitle', 'addon', 'releaseInfo', 'runtime', 'imdbRating', 'status', 'awards', 'streamingServer']) {
    ...
    if (Array.isArray(m.streamAddons)) fields.metadata.streamAddons = m.streamAddons.filter(v => typeof v === 'string' && /^https?:\/\//.test(v)).slice(0, 100).map(v => v.slice(0, 2048));
    ...
    if (ContentPolicy.restricted(m)) fields.metadata.adult = true;
  ...
  if (ContentPolicy.restricted({ ...body, metadata: fields.metadata }) && fields.category !== 'porn') fields.category = 'plus18';
  return fields;
}
...
function classify(url, info) {
  ...
  if (ContentPolicy.restricted(info)) return 'plus18';
